...
A second finding provides positive evidence for patterning's mode decomposition. Tokenizer changes and corpus changes produce orthogonal structural responses. Changing the corpus from FineWeb to the structok corpus (33% web, 35% structured) while keeping the same standard BPE tokenizer shifts the delimiter mode (+57 heads) without touching spacing (-11 heads). Changing the tokenizer (adding barriers) while keeping the same FineWeb corpus shifts the spacing mode (-170 to -183 heads). These are different susceptibility modes targeted by different interventions, exactly as patterning predicts: different data perturbations couple to different principal structures through different columns of the susceptibility matrix. This orthogonality is the first empirical demonstration of mode-specific structural responses at 410M scale.
