...
### 5.5 Empirical Constraints on the Patterning Framework at Scale

Wang and Murfet (2026) proposed patterning as the dual of interpretability: given a target internal structure, derive the optimal data distribution intervention through the fundamental equation $dh_{opt} = \chi^\dagger d\mu_{target}^\infty$. This framework assumes an approximately linear relationship between data distribution shifts and structural responses. Our multi-tokenizer data provides the first opportunity to test this assumption at 410M scale.

We compare two interventions: structured barriers (protecting 16 characters with a total adversarial surface of 18,647 mergeable words across 43 tokenizers) and NL barriers (protecting 10 characters with a smaller total surface of 13,057 words, 0.70x). If the linear assumption holds, the structural response should scale with the distribution shift magnitude: structured barriers should produce a larger response.

The opposite occurs. NL barriers eliminate more spacing heads (183 vs 170), produce more total head reallocation (398 vs 340 heads moved), and generate more bracket specialists (56 vs 35) and induction heads (19 vs 10). The structural response is 1.08x to 1.90x larger despite a 0.70x distribution shift. Wang and Murfet (2026) note in their parenthesis balancing experiment that the Almost Equal distribution "behaves unexpectedly" and suggest this "may reflect the limits of the linear response approximation: our interventions are finite perturbations, not infinitesimal ones" (Section 4.4). Our data confirms their suspicion at 140x scale: the linear approximation does not predict the spacing response from aggregate distribution shift size alone. What determines the response is which characters are protected: NL barrier characters (period, hyphen, apostrophe) co-occur with whitespace constantly in prose, making them disproportionately effective despite a smaller total adversarial surface. The structural response depends on the character-specific interaction between barrier characters and corpus statistics, not on the aggregate perturbation magnitude.

A second finding provides positive evidence for patterning's mode decomposition. The patterning framework decomposes the susceptibility matrix into independent modes via SVD, predicting that different data perturbations should couple to different internal structures independently. Our multi-tokenizer, multi-corpus experimental design provides a direct empirical test: changing the tokenizer (adding barriers) while keeping the same FineWeb corpus shifts the spacing mode (-170 to -183 heads) without proportionally affecting delimiters, while changing the corpus (FineWeb to structok) while keeping the same standard BPE tokenizer shifts the delimiter mode (+57 heads) without touching spacing (-11 heads). These are orthogonal structural responses to orthogonal data interventions, exactly as the mode decomposition predicts. This is the first empirical demonstration of mode-specific structural responses at 410M scale, obtained through controlled experiments rather than susceptibility estimation.

Together, these findings suggest that patterning's mode decomposition holds at realistic scale (different interventions target different modes), but the linear magnitude prediction requires refinement: the structural response depends on the character-specific interaction between barrier characters and training corpus statistics, not on the aggregate distribution shift.

## 6. Limitations

**Taxonomy coverage.** The 7-behavior taxonomy captures the major positional and structural specializations but likely misses content-based and syntactic behaviors. The merge-barrier comparison model has 95 unclassified heads (24.7%), all with excess scores below 0.02 across every measured behavior. These heads are not secretly specializing in a behavior we probe for; they show no above-threshold response to any positional or delimiter pattern. They likely perform content-based processing (attending by embedding similarity rather than position) or syntactic processing (grammatical dependencies) that positional attention probes cannot detect. The baseline model has only 10 unclassified heads (2.6%), not because baseline heads are better classified, but because spacing absorbs nearly half the taxonomy's classification capacity. The imbalance (95 vs 10) itself reinforces the capacity tax finding: the merge-barrier model has freed enough heads from spacing recovery that many can pursue specializations our taxonomy does not measure. Activation patching with targeted linguistic input pairs (subject-verb agreement, coreference, semantic plausibility, local context, clause boundaries) did not differentiate these heads at 410M scale: all behavioral probes produced near-zero logit shifts, indicating that the model has not developed the linguistic capabilities these probes test for. The freed heads are performing processing below the resolution of behavioral probes at this model size, likely content similarity via embedding space, local windowing, or diffuse contextual processing. Sparse autoencoder analysis or testing at larger scale may reveal their function.
