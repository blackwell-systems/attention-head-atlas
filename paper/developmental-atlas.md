...
We present the first comprehensive developmental atlas of attention head specialization at realistic scale, tracking 384 heads across 24 layers in a GPT-NeoX 410M model from random initialization through 20,000 training steps. Using 131 checkpoints per run, 7 behavior types (positional, P0 sink, induction, delimiter, bracket, duplicate, and spacing, plus entropy and dormancy as auxiliary metrics), and 6 probe texts, we map the full lifecycle of head specialization across six training runs: four on a web text corpus (standard BPE baseline, structured-data merge barriers, seed variation, natural-language merge barriers) and two on a mixed structured-data corpus (standard BPE and structured merge barriers). Our excess score methodology (subtracting step-0 base rates from raw attention scores) corrects for probe-induced inflation, reducing apparent delimiter specialists from 172 to 74 in the baseline model.

Three principal findings emerge. First, approximately half of all attention heads in standard BPE models are consumed by mandatory boundary recovery. In our 410M models, 183 of 384 heads (47.7%) become spacing specialists, dedicating capacity to whitespace boundary recovery. Ablation proves this recovery is essential (+64% degradation when removed), but merge barriers eliminate the need for it entirely (0 spacing heads with NL barriers). This is a capacity tax, not a design choice: the model is forced to repair damage that BPE inflicts on whitespace boundaries. The count is deterministic across seeds, confirming the "spacing fin" prediction of Wang et al. (2025b) at 410M scale.

Second, position-zero (P0) sinking is a failure cascade, not a design choice. Prior to adding spacing to the probe taxonomy, the P0 count appeared to be 96. Adding spacing revealed that 54 of those heads were spacing specialists, reducing the genuine P0 count to 29 to 32 (8.3%). This measurement correction demonstrates the importance of comprehensive behavior taxonomies; omitting a single behavior type can inflate other categories by over 3x. Tracking the genuine P0 heads backward through training confirms the cascade: heads attempt specialization, fail because BPE merges corrupt delimiter boundaries, and gradually collapse into the P0 sink mechanism. All genuine P0 heads are isolated from co-specializing circuits, establishing circuits as developmentally protective.
...
The structured-barrier model shows the highest unclassified count (95, 24.7%). With spacing measured separately, many heads show no single dominant behavior above base rate. These are truly generalist heads rather than misclassified specialists.
...
The falsified spacing prediction is the strongest result. We expected the structured content to compete with spacing for head capacity. Instead, spacing persists at approximately the same level while delimiter heads grow at the expense of positional_prev heads. Predictions were registered in the experiment design document prior to running.
