...
The opposite occurs. NL barriers eliminate more spacing heads (183 vs 170), produce more total head reallocation (398 vs 340 heads moved), and generate more bracket specialists (56 vs 35) and induction heads (19 vs 10). The structural response is 1.08x to 1.90x larger despite a 0.70x distribution shift. The linear approximation does not predict the spacing response from raw distribution shift size alone. What determines the spacing response is not how many mergeable words are affected but which characters are protected: NL barrier characters (period, hyphen, apostrophe) co-occur with whitespace constantly in prose, making them disproportionately effective at preventing whitespace boundary corruption despite a smaller total adversarial surface.
