...
**P0 heads are genuinely useless (causal proof).** Removing 31 to 40 P0 heads produces negligible PPL change across all four models and both architectures (+1.4% NeoX baseline, -3.4% Llama baseline, +0.5% structok baseline, +0.2% comparison). This converts the correlational finding (100% circuit isolation, Section 4.5) into causal evidence: P0 heads contribute nothing to model performance on either architecture. They are a pure failure mode, confirming the answer to Gu et al.'s first open question with a direct intervention rather than observational data alone.
