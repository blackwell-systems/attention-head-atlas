...
def run_ablation(model, tokenizer, texts, heads_to_ablate, label, device):
    ...
    ablated = zero_heads(model, heads_to_ablate)
...
def main():
    ...
    print("Loading model...")
    model, tokenizer = load_model(cp_path, args.tokenizer, args.device)
